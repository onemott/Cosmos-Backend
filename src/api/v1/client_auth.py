...
@router.post(
    ...
    description="Change the current client's password.",
)
async def change_password(
    ...
    if not verify_password(request.current_password, client_user.hashed_password):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    # Update password
    client_user.hashed_password = hash_password(request.new_password)
    await db.commit()
